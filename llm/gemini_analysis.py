...
def ask_gemini(prompt: str, model: genai.GenerativeModel) -> Dict:
    """Send a prompt to Gemini 2.5 Pro, return parsed JSON (with enhanced retry logic)."""
    ...
    raise RuntimeError(f"Gemini request failed after {MAX_RETRIES} attempts") from last_error
...
def analyze(peaks: List[dict]) -> List[dict]:
    ...
    print(f"Analyzing {len(peaks)} peaks with Gemini 2.5 Pro...")
    
    for i, peak in enumerate(peaks, 1):
        print(f"Processing peak {i}/{len(peaks)} at {peak['position']:.2f} cm⁻¹...")
        
        try:
            result = ask_gemini(build_prompt(peak), model)
...
def main() -> None:
    print("Raman Peak Analysis with Gemini 2.5 Pro (Enhanced)")
    print("=" * 50)
