...
OUT_FILE  = ROOT / "results" / "peaks_analysis_claude_haiku35_updated_prompt.json"
SUMMARY_FILE = ROOT / "results" / "summary_claude_haiku35.json"
...
def ask_claude(prompt: str, client: anthropic.Anthropic) -> Dict:
    """Send a prompt to Claude Opus 4, return parsed JSON (with enhanced retry logic)."""
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                ...
                print(f"Rate limit error (attempt {attempt}/{MAX_RETRIES}), retrying in {wait_time}s...")
    ...
    raise RuntimeError(f"Claude request failed after {MAX_RETRIES} attempts") from last_error
...
def analyze(peaks: List[dict]) -> List[dict]:
    ...
    print(f"Analyzing {len(peaks)} peaks with Claude Opus 4...")
    
    for i, peak in enumerate(peaks, 1):
        print(f"Processing peak {i}/{len(peaks)} at {peak['position']:.2f} cm⁻¹...")
        
        try:
            result = ask_claude(build_prompt(peak), client)
...
def main() -> None:
    print("Raman Peak Analysis with Claude Opus 4 (Enhanced)")
    print("=" * 50)
